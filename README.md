...
Необязательно добавьте раздел `balance` для точной настройки экономики и формул прогрессии. Все параметры имеют значения по
умолчанию, поэтому можно переопределять только интересующие числа. Например:

```json
"balance": {
  "reward_base": 0.95,
  "job_difficulty_pay": 24,
  "renown_success_base": 6
}
```

Такой блок усилит базовые выплаты и ускорит рост славы, не требуя правок исходного кода.
